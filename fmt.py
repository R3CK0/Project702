...
class FMTStar2D:
    def __init__(self, environement, game_engine, free_space_volume=None, radius_multiplier=1.1, dimension=2):
        ...
        if free_space_volume is None:
            self.free_space_volume = (environement['width']-2*environement['grid_size']) * (environement['height']-2*environement['grid_size']) - len(self.game_engine.obstacles) * environement['grid_size']**2
        else:
            self.free_space_volume = free_space_volume

    def find_path(self, start_pos, end_pos, progress=False, K = 1000):
        closed_set = set()
